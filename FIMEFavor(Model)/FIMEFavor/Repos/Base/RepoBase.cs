...
namespace FIMEFavor.DAL.Repos.Base
{
    public abstract class RepoBase<T> : IDisposable, IRepo<T> where T : EntityBase, new()
    {
        protected readonly FimeContext Db;
        protected RepoBase()
        {
            Db = new FimeContext();
            Table = Db.Set<T>();
        }
        protected RepoBase(DbContextOptions<FimeContext> options)
        {
            Db = new FimeContext(options);
            Table = Db.Set<T>();
        }

        protected DbSet<T> Table;
        public FimeContext Context => Db;
